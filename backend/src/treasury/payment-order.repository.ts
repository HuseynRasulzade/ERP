...
export const PAYMENT_ORDER_TYPE = 'PAYMENT_ORDER';

@Injectable()
export class PaymentOrderRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.paymentOrder.findFirst({ where: { id, tenantId } });
  ...
  async create(): Promise<BaseDocumentFields> {
    // PaymentOrder is always created through PaymentOrderService — never
    // through the generic create-based-on path.
    throw new Error('Use PaymentOrderService.create() — PaymentOrder is never created generically');
  }
